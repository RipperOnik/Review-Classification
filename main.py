...
# @st.cache(suppress_st_warning=True)
# def load_data():
#     data = pd.read_csv('movie_data.csv', nrows = 500)
#     return data
...
class Model:
    def __init__(self, name, scores, image):
        self.name = name
        self.scores = scores
        self.image = image
...
def init_models():
    ...
    for model_name in models_list:
        models_array.append(Model(model_name, pd.read_csv(model_name+".csv", index_col=[0]), Image.open(model_name+".png")))
    return models_array

def print_static_models():
    for model_name in models_select:
        for model in models:
            if model_name == model.name:
                graphCol.image(model.image)
                graphCol.write(model.scores)
